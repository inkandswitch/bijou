...
- `encode_injective` / `decode_canonical` — `encode` is a bijection
  between `[0, 256 ^ tiers)` and the accepted byte strings.
- `encode_lex_iff` — lexicographic byte order equals numeric order.

`Bijou.Instances` instantiates the three specified variants and checks
every test vector from the SPEC documents by reduction.
-/
