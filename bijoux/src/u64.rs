...
//! in a shorter form. An "overlong" spelling like `[0xF8, 0x00]` is not a
//! redundant encoding of `0` that must be rejected — it *means* `248`,
//! because the decoder adds the tier offset. There is no check to forget.
//!
//! The decoder still has two (non-canonicality) runtime error conditions:
//! buffer underflow, and arithmetic overflow on tier `8` — the top tier's
...
//!
//! let (value, len) = bijoux::u64::decode(&buf).unwrap();
//! assert_eq!(value, 300);
//! assert_eq!(len, 2);
//! ```
//!
...
#![cfg_attr(not(feature = "u64"), allow(unreachable_pub))]
...
///
/// ```
/// assert_eq!(bijoux::u64::encoded_len(0), 1);
/// assert_eq!(bijoux::u64::encoded_len(247), 1);
/// assert_eq!(bijoux::u64::encoded_len(248), 2);
/// assert_eq!(bijoux::u64::encoded_len(503), 2);
/// assert_eq!(bijoux::u64::encoded_len(504), 3);
/// assert_eq!(bijoux::u64::encoded_len(u64::MAX), 9);
/// ```
#[inline]
#[must_use]
...
/// ```
/// let mut buf = Vec::new();
/// bijoux::u64::encode(42, &mut buf);
/// assert_eq!(buf, [0x2A]);
///
/// buf.clear();
/// bijoux::u64::encode(248, &mut buf);
...
impl EncodedU64 {
    ...
    #[inline]
    #[must_use]
    #[cfg_attr(not(feature = "u64"), allow(clippy::trivially_copy_pass_by_ref))]
...
///
/// ```
/// let enc = bijoux::u64::encoded_bytes(300);
...
/// ```
/// // Single-byte value
/// let (v, n) = bijoux::u64::decode(&[0x2A]).unwrap();
/// assert_eq!((v, n), (42, 1));
///
/// // Multi-byte value with trailing data
/// let (v, n) = bijoux::u64::decode(&[0xF8, 0x34, 0xFF]).unwrap();
/// assert_eq!((v, n), (300, 2));
/// ```
#[inline]
...
///
/// ```
/// use bijoux::u64::{decode_iter, encode};
///
/// let mut buf = Vec::new();
...
///
/// ```
/// use bijoux::u64::{decode_all, encode};
///
/// let mut buf = Vec::new();
...
/// ```
#[cfg_attr(not(feature = "u64"), allow(dead_code))]
