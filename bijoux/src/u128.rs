...
//! `[0xF0, 0x00]` is not a redundant encoding of `0` that must be rejected —
//! it *means* `240`, because the decoder adds the tier offset. There is no
//! check to forget.
//!
//! The decoder still has two (non-canonicality) runtime error conditions:
//! buffer underflow, and arithmetic overflow on tier `16` — the top tier's
...
//! let mut buf = Vec::new();
//! bijoux::u128::encode(42, &mut buf);
//! assert_eq!(buf, [0x2A]);
//!
//! // First multi-byte value: tag 0xF0 plus 1 payload byte.
//! let mut buf = Vec::new();
//! bijoux::u128::encode(240, &mut buf);
//! assert_eq!(buf, [0xF0, 0x00]);
//!
//! // Round-trip through `decode`.
//! let mut buf = Vec::new();
//! bijoux::u128::encode(u128::MAX, &mut buf);
//! let (value, len) = bijoux::u128::decode(&buf).unwrap();
...
#![cfg_attr(not(feature = "u128"), allow(unreachable_pub))]
...
///
/// ```
/// assert_eq!(bijoux::u128::encoded_len(0), 1);
/// assert_eq!(bijoux::u128::encoded_len(239), 1);
/// assert_eq!(bijoux::u128::encoded_len(240), 2);
/// assert_eq!(bijoux::u128::encoded_len(495), 2);
/// assert_eq!(bijoux::u128::encoded_len(496), 3);
/// assert_eq!(bijoux::u128::encoded_len(u128::MAX), 17);
/// ```
#[inline]
#[must_use]
...
pub struct EncodedU128 {
    buf: [u8; MAX_BYTES],
    /// Invariant: `len <= MAX_BYTES`. Stored as `u8` because the value
    /// is always in `1..=17`; the smaller width makes `EncodedU128`
...
impl PartialEq for EncodedU128 {
    #[inline]
...
impl Eq for EncodedU128 {}

impl core::hash::Hash for EncodedU128 {
    #[inline]
...
impl PartialOrd for EncodedU128 {
    #[inline]
...
impl EncodedU128 {
    ...
    #[inline]
    #[must_use]
    #[cfg_attr(not(feature = "u128"), allow(clippy::trivially_copy_pass_by_ref))]
...
impl core::ops::Deref for EncodedU128 {
    type Target = [u8];
...
impl core::borrow::Borrow<[u8]> for EncodedU128 {
    #[inline]
    fn borrow(&self) -> &[u8] {
        self.as_slice()
    }
...
impl IntoIterator for EncodedU128 {
    type Item = u8;
...
impl<'a> IntoIterator for &'a EncodedU128 {
    type Item = &'a u8;
...
/// Encodes `value` as a stack-allocated [`EncodedU128`].
///
...
///
/// ```
/// let enc = bijoux::u128::encoded_bytes(240);
...
#[must_use]
#[allow(clippy::cast_possible_truncation, clippy::indexing_slicing)]
pub const fn encoded_bytes(value: u128) -> EncodedU128 {
    if value < BOUNDS[0] {
        return EncodedU128 {
            buf: [
                (value & 0xFF) as u8,
...
/// ```
/// // Single-byte value
/// let (v, n) = bijoux::u128::decode(&[0x2A]).unwrap();
/// assert_eq!((v, n), (42, 1));
///
/// // Multi-byte value with trailing data
/// let (v, n) = bijoux::u128::decode(&[0xF0, 0x00, 0xFF]).unwrap();
/// assert_eq!((v, n), (240, 2));
/// ```
#[inline]
...
///
/// ```
/// use bijoux::u128::{decode_iter, encode};
///
/// let mut buf = Vec::new();
...
///
/// ```
/// use bijoux::u128::{decode_all, encode};
///
/// let mut buf = Vec::new();
...
/// ```
#[cfg_attr(not(feature = "u128"), allow(dead_code))]
...
mod tests {
    ...
    mod encoded_bytes {
        ...
        fn borrow_impl() {
            ...
            let mut map: BTreeMap<EncodedU128, &'static str> = BTreeMap::new();
