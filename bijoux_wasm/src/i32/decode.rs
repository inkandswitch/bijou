...
pub enum WasmInputError {
    ...
    #[error("bijou32s: expected a Uint8Array")]
    WrongType,
}
...
) -> Result<WasmDecodedI32, JsValue> {
    let bytes = bytes_from_js(bytes)?;
    let (value, bytes_read) = bijoux::i32::decode(&bytes).map_err(WasmDecodeError::from)?;
...
/// `Vec<i32>` lets wasm-bindgen marshal the result as a typed
/// `Int32Array`, which is denser and faster than a JS array of
/// `Number`s for large batches.
///
/// # Errors
///
/// Throws a JS native `TypeError` if `bytes` is not a `Uint8Array`.
/// Throws a JS `Error` with `name === "Bijou32sDecodeError"` (same
/// shape as [`decode_i32`]) if any element fails to decode. The
...
///
/// ```js
/// import { encodeI32, decodeAllI32 } from "@inkandswitch/bijoux";
/// const buf = new Uint8Array([...encodeI32(42), ...encodeI32(300), ...encodeI32(65535)]);
/// const values = decodeAllI32(buf);
/// // values is Int32Array([42, 300, 65535])
/// ```
#[wasm_bindgen(js_name = decodeAllI32)]
pub fn decode_all_i32(
    #[wasm_bindgen(unchecked_param_type = "Uint8Array")] bytes: &JsValue,
) -> Result<Vec<i32>, JsValue> {
    let bytes = bytes_from_js(bytes)?;
    let mut out = Vec::new();
    for result in bijoux::i32::decode_iter(&bytes) {
...
#[wasm_bindgen(js_name = DecodedI32)]
#[derive(Debug, Clone, Copy)]
pub struct WasmDecodedI32 {
    value: i32,
    bytes_read: usize,
}

#[wasm_bindgen(js_class = DecodedI32)]
impl WasmDecodedI32 {
    /// The decoded value.
    #[must_use]
    #[wasm_bindgen(getter)]
...
/// `"Bijou32sDecodeError"`.
///
/// We wrap [`bijoux::i32::DecodeError`] in a newtype so we can implement
...
impl From<WasmDecodeError> for JsValue {
    fn from(err: WasmDecodeError) -> Self {
        ...
        js_err.set_name("Bijou32sDecodeError");
        js_err.into()
    }
}
