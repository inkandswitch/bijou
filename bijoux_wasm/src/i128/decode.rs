...
pub enum WasmInputError {
    ...
    #[error("bijou128s: expected a Uint8Array")]
    WrongType,
}
...
) -> Result<WasmDecodedI128, JsValue> {
    let bytes = bytes_from_js(bytes)?;
    let (value, bytes_read) = bijoux::i128::decode(&bytes).map_err(WasmDecodeError::from)?;
...
///
/// Equivalent to calling [`decode_i128`] in a loop, advancing by
/// `bytesRead` after each call until the buffer is empty.
///
/// # Errors
///
/// Throws a JS native `TypeError` if `bytes` is not a `Uint8Array`.
/// Throws a JS `Error` with `name === "Bijou128sDecodeError"` (same
/// shape as [`decode_i128`]) if any element fails to decode. The
...
///
/// ```js
/// import { encodeI128, decodeAllI128 } from "@inkandswitch/bijoux";
/// const buf = new Uint8Array([...encodeI128(42n), ...encodeI128(500n), ...encodeI128(65535n)]);
/// const values = decodeAllI128(buf);
/// // values is [42n, 500n, 65535n]
/// ```
#[wasm_bindgen(js_name = decodeAllI128)]
pub fn decode_all_i128(
    #[wasm_bindgen(unchecked_param_type = "Uint8Array")] bytes: &JsValue,
) -> Result<js_sys::Array, JsValue> {
    let bytes = bytes_from_js(bytes)?;
    let out = js_sys::Array::new();
    for result in bijoux::i128::decode_iter(&bytes) {
        let value: i128 = result.map_err(WasmDecodeError::from)?;
...
#[wasm_bindgen(js_name = DecodedI128)]
#[derive(Debug, Clone)]
#[allow(missing_copy_implementations)] // intentional per the wasm-bindgen blog post
pub struct WasmDecodedI128 {
    value: i128,
    bytes_read: usize,
}

#[wasm_bindgen(js_class = DecodedI128)]
impl WasmDecodedI128 {
    /// The decoded value.
    #[must_use]
    #[wasm_bindgen(getter)]
...
///
/// We wrap [`bijoux::i128::DecodeError`] in a newtype so we can implement
...
impl From<WasmDecodeError> for JsValue {
    fn from(err: WasmDecodeError) -> Self {
        ...
        js_err.set_name("Bijou128sDecodeError");
        js_err.into()
    }
}
