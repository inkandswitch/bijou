...
pub enum WasmInputError {
    ...
    #[error("bijou64s: expected a Uint8Array")]
    WrongType,
}
...
/// Decodes a `bijou64s` from the front of `bytes`.
///
/// Returns a [`WasmDecodedI64`] carrying the value plus the
...
/// Throws a JS `Error` with `name === "Bijou64sDecodeError"` if `bytes`
/// is too short for the encoding indicated by its tag byte, or if a
/// tier-8 payload would overflow the zigzag `u64` range. See [`WasmDecodeError`].
///
/// # JS
///
/// ```js
/// import { decodeI64 } from "@inkandswitch/bijoux";
/// const { value, bytesRead } = decodeI64(new Uint8Array([0xF8, 0x34, 0xFF]));
/// // value === 150n (unzigzag(300n)), bytesRead === 2
/// decodeI64([0xF8, 0x34]); // throws TypeError (plain Array, not Uint8Array)
/// ```
#[wasm_bindgen(js_name = decodeI64)]
pub fn decode_i64(
    #[wasm_bindgen(unchecked_param_type = "Uint8Array")] bytes: &JsValue,
) -> Result<WasmDecodedI64, JsValue> {
    let bytes = bytes_from_js(bytes)?;
    let (value, bytes_read) = bijoux::i64::decode(&bytes).map_err(WasmDecodeError::from)?;
    Ok(WasmDecodedI64 { value, bytes_read })
}

/// Decodes every `bijou64s`-encoded value in `bytes`, returning them as
/// a `BigInt64Array`.
///
/// Equivalent to calling [`decode_i64`] in a loop, advancing by
/// `bytesRead` after each call until the buffer is empty. Returning
/// `Vec<i64>` lets wasm-bindgen marshal the result as a typed
/// `BigInt64Array`, which is denser and faster than a JS array of
/// `bigint`s for large batches.
///
/// # Errors
///
/// Throws a JS native `TypeError` if `bytes` is not a `Uint8Array`.
/// Throws a JS `Error` with `name === "Bijou64sDecodeError"` (same
/// shape as [`decode_i64`]) if any element fails to decode. The
...
///
/// ```js
/// import { encodeI64, decodeAllI64 } from "@inkandswitch/bijoux";
/// const buf = new Uint8Array([...encodeI64(42n), ...encodeI64(300n), ...encodeI64(65535n)]);
/// const values = decodeAllI64(buf);
/// // values is BigInt64Array([42n, 300n, 65535n])
/// ```
#[wasm_bindgen(js_name = decodeAllI64)]
pub fn decode_all_i64(
    #[wasm_bindgen(unchecked_param_type = "Uint8Array")] bytes: &JsValue,
) -> Result<Vec<i64>, JsValue> {
    let bytes = bytes_from_js(bytes)?;
    let mut out = Vec::new();
    for result in bijoux::i64::decode_iter(&bytes) {
...
///
/// Returned by [`decode_i64`]. Exposes `value` (the decoded `i64`, JS
...
#[wasm_bindgen(js_name = DecodedI64)]
#[derive(Debug, Clone)]
#[allow(missing_copy_implementations)] // intentional per the wasm-bindgen blog post
pub struct WasmDecodedI64 {
    value: i64,
    bytes_read: usize,
}

#[wasm_bindgen(js_class = DecodedI64)]
impl WasmDecodedI64 {
    /// The decoded value.
    #[must_use]
    #[wasm_bindgen(getter)]
...
/// `"Bijou64sDecodeError"`.
///
/// We wrap [`bijoux::i64::DecodeError`] in a newtype so we can implement
...
impl From<WasmDecodeError> for JsValue {
    fn from(err: WasmDecodeError) -> Self {
        ...
        js_err.set_name("Bijou64sDecodeError");
        js_err.into()
    }
}
