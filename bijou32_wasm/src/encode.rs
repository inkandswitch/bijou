...
///
/// ```js
/// import { encodedLen } from "bijou32";
/// encodedLen(0);          // 1
/// encodedLen(251);        // 1
/// encodedLen(252);        // 2
/// encodedLen(2**32 - 1);  // 5 (u32::MAX)
/// encodedLen(2**32);      // throws RangeError
/// encodedLen(-1);         // throws RangeError
/// encodedLen(1.5);        // throws TypeError (not an integer)
/// encodedLen(42n);        // throws TypeError (bigint, not number)
/// ```
#[wasm_bindgen(js_name = encodedLen)]
pub fn encoded_len(value: &JsValue) -> Result<usize, WasmNumberError> {
    Ok(bijou32::encoded_len(jsvalue_to_u32(value)?))
}
...
///
/// ```js
/// import { encode } from "bijou32";
/// encode(42);          // Uint8Array([0x2A])
/// encode(300);         // Uint8Array([0xFC, 0x30])
/// encode(2**32);       // throws RangeError
/// encode(-1);          // throws RangeError
/// encode(1.5);         // throws TypeError (not an integer)
/// ```
#[wasm_bindgen]
pub fn encode(value: &JsValue) -> Result<Vec<u8>, WasmNumberError> {
    let v = jsvalue_to_u32(value)?;
    let mut buf = Vec::with_capacity(bijou32::MAX_BYTES);
    bijou32::encode(v, &mut buf);
    Ok(buf)
}
